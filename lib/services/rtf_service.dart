...
class RtfService {
  ...
  Future<RtfParseResult> parseRtfContentWithFormatting(String rtfContent) async {
    try {
      final parser = RtfParser();
  ...
  Future<List<RtfFile>> getRecentFiles() async {
    try {
      ...
      for (var fileEntity in files) {
        if (fileEntity is File && fileEntity.path.toLowerCase().endsWith('.rtf')) {
          final fileStats = await fileEntity.stat();
          rtfFiles.add(RtfFile(
